...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `mustResetPassword` TINYINT(1) NOT NULL DEFAULT 1,
  `createdAt` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `sales` (
  ...
  `branchId` VARCHAR(64) NOT NULL,
  `cashierId` VARCHAR(64) NULL,
  `cashierName` VARCHAR(191) NULL,
  `customerName` VARCHAR(191) NOT NULL DEFAULT 'Walk-in',
  `customerAddress` TEXT NULL,
  `customerTin` VARCHAR(100) NULL,
  `businessStyle` VARCHAR(255) NULL,
...
CREATE TABLE IF NOT EXISTS `active_sessions` (
  ...
  `branchName` VARCHAR(191) NULL,
  `lastActive` DATETIME NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `userAgent` TEXT NULL,
