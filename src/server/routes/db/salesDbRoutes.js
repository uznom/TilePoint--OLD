...
router.post(['/sales', '/mysql/sales', '/sqlite/sales'], express.json(), async (req, res) => {
  ...
  if (!getIsMysqlActive()) {
    try {
      ...
      if (!alaDb.tables.tp_sales) alaDb.tables.tp_sales = { data: [] };
      if (!alaDb.tables.tp_sale_items) alaDb.tables.tp_sale_items = { data: [] };
      ...
      const existingIdx = alaDb.tables.tp_sales.data.findIndex(s => s.id === sale.id);
      const saleRow = {
        id: sale.id,
      ...
      if (existingIdx >= 0) {
        alaDb.tables.tp_sales.data[existingIdx] = saleRow;
      } else {
        alaDb.tables.tp_sales.data.push(saleRow);
      }

      if (Array.isArray(sale.items)) {
        for (const item of sale.items) {
          const itemIdx = alaDb.tables.tp_sale_items.data.findIndex(i => i.id === item.id);
          const itemRow = {
          ...
          if (itemIdx >= 0) {
            alaDb.tables.tp_sale_items.data[itemIdx] = itemRow;
          } else {
            alaDb.tables.tp_sale_items.data.push(itemRow);
          }
        }
      }
...
router.get('/sales/lookup', async (req, res) => {
  ...
  try {
    if (!getIsMysqlActive()) {
      const alaDb = getAlasqlDb();
      let data = alaDb.tables.tp_sales?.data || [];
    ...
    let sql = 'SELECT * FROM tp_sales WHERE 1=1';
    const params = [];
...
router.get('/shifts/:shiftId/summary', async (req, res) => {
  ...
  try {
    if (!getIsMysqlActive()) {
      const alaDb = getAlasqlDb();
      const sales = (alaDb.tables.tp_sales?.data || []).filter(s => s.shiftId === shiftId && !s.isDeleted);
