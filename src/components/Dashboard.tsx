...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const [activeDrilldown, setActiveDrilldown] = useState<
    "none" | "low" | "critical" | "out_of_stock" | "products"
  >("none");
  ...
  const [branchSortKey, setBranchSortKey] = useState<
    "sales" | "growth" | "name" | "staff"
  >("sales");
  const [branchSortOrder, setBranchSortOrder] = useState<"asc" | "desc">(
    "desc",
  );
  const [bestsellerSortBy, setBestsellerSortBy] = useState<"qty" | "revenue">(
    "qty",
  );
  ...
  const [weeklyMetric, setWeeklyMetric] = useState<
    "revenue" | "orders" | "boxes"
  >("revenue");
  const [selectedWeeklyDay, setSelectedWeeklyDay] = useState<number | null>(
    null,
  );

  // 6-Month Enterprise Revenue Wave parameters
  const [waveStyle, setWaveStyle] = useState<"spline" | "step" | "linear">(
    "spline",
  );
  ...
  const [corporateTarget, setCorporateTarget] = useState<number>(() => {
    return (
      Number(localStorage.getItem("tilepoint_corporate_target_v1")) || 10000000
    );
  });
  const [isEditingCorporateTarget, setIsEditingCorporateTarget] =
    useState<boolean>(false);
  const [tempCorporateTargetInput, setTempCorporateTargetInput] =
    useState<string>("");
  ...
  const [activeDailyPaymentFilter, setActiveDailyPaymentFilter] =
    useState<string>("all");
  const [showAllDailyTransactions, setShowAllDailyTransactions] =
    useState<boolean>(false);
  const [showDailySalesMonitor, setShowDailySalesMonitor] =
  ...
  const filteredPurchaseOrders =
    currentUser.role === UserRole.ADMIN
      ...
      : purchaseOrders.filter(
          (po) => po.branchId === currentUser.branchAssignmentId,
        );
  ...
  const getProductStockForCurrentContext = (pId: string) => {
    if (activeBranchId) {
      const bs = branchStock.find(
        (item) => item.productId === pId && item.branchId === activeBranchId,
      );
  ...
  const todaySalesItems = filteredSales.filter(
    (s) => s.createdAt.startsWith(todayStr) && !s.isDeleted,
  );
  const computedTodaySales = todaySalesItems.reduce(
  ...
  const weeklySalesItems = filteredSales.filter(
    (s) => new Date(s.createdAt) >= sevenDaysAgo && !s.isDeleted,
  );
  const computedWeeklySales = weeklySalesItems.reduce(
    (acc, curr) => acc + curr.grandTotal,
    0,
  );
  ...
  const monthlySalesItems = filteredSales.filter(
    (s) => s.createdAt.startsWith(currentMonthStr) && !s.isDeleted,
  );
  const computedMonthlyRevenue = monthlySalesItems.reduce(
    (acc, curr) => acc + curr.grandTotal,
    0,
  );

  // Corporate aggregate metric calculation (All branches, regardless of assignment)
  const corporateTodaySales = sales
    .filter((s) => s.createdAt.startsWith(todayStr) && !s.isDeleted)
    .reduce((acc, s) => acc + s.grandTotal, 0);
  const corporateMonthlyRevenue = sales
    .filter((s) => s.createdAt.startsWith(currentMonthStr) && !s.isDeleted)
    .reduce((acc, s) => acc + s.grandTotal, 0);
  ...
  const totalInventoryCostValue = branchStock
    ...
    .reduce((acc, bs) => {
      const product = products.find(
        (p) => p.id === bs.productId && !p.isDeleted,
      );
  ...
  const outOfStockProducts = activeProducts.filter(
    (p) => getProductStockForCurrentContext(p.id) === 0,
  );
  ...
  const pendingOrders = filteredPurchaseOrders.filter(
  ...
  const getWeeklyChartData = () => {
    ...
    return days.map((day, idx) => {
      ...
      const daySales = filteredSales.filter(
        (s) => s.createdAt.startsWith(targetDateStr) && !s.isDeleted,
      );
      ...
      if (weeklyMetric === "revenue") {
        ...
        liveValue = saleItems
          .filter((si) => saleIds.has(si.saleId) && !si.isDeleted)
  ...
  const getMonthlyChartData = () => {
    ...
    let list = months.map((month, idx) => {
      ...
      const monthSales = filteredSales.filter(
        (s) => s.createdAt.startsWith(monthPrefix) && !s.isDeleted,
      );
    ...
    if (forecastEnabled) {
      ...
      forecastMonths.forEach((m, idx) => {
        const forecastedRevenue = Math.round(
          currentVal * (1 + 0.04 * (idx + 1)) + trendVal,
        );
        list.push({
          month: m,
  ...
  const getMonthlyRatio = (val: number) => {
    if (!maxMonthlyAmount || isNaN(maxMonthlyAmount) || maxMonthlyAmount <= 0)
      return 0;
  ...
  const generateSvgPaths = (
    data: { month: string; revenue: number; isPredicted?: boolean }[],
  ) => {
    const width = 560;
    const height = 150;
  ...
  const categorySummary = activeProducts.reduce(
    (acc, p) => {
  ...
  const activeSalesIds = new Set(
    filteredSales.filter((s) => !s.isDeleted).map((s) => s.id),
  );
  const filteredSaleItems = saleItems.filter(
    (si) => activeSalesIds.has(si.saleId) && !si.isDeleted,
  );

  // Group sale items by product identifier
  const productSalesMap = filteredSaleItems.reduce(
    (acc, item) => {
  ...
  const bestsellingProducts = (
    Object.values(productSalesMap) as {
      productId: string;
      productName: string;
      quantitySold: number;
      revenue: number;
  ...
  const handleExecuteRedistribution = (
    candidate: (typeof slowMovingCandidates)[0],
  ) => {
    try {
      ...
      createStockTransfer(
        candidate.branchId,
        candidate.targetBranchId,
        "Redistribution",
        items,
        reason,
      );
      showToastMsg(
        `Created redistribution transfer request of ${candidate.productName} from ${candidate.branchName}!`,
        "success",
      );
    } catch (e) {
      console.error(e);
      showToastMsg(
        "Failed to execute stock redistribution transfer request.",
        "error",
      );
    }
  };
  ...
  const handleApproveTransfer = (id: string, transferNo: string) => {
    try {
      ...
      showToastMsg(
        `Approved and Shipped Transfer Request ${transferNo}! Assets routed.`,
        "success",
      );
    } catch (e) {
      console.error(e);
  ...
  const handleDeclineTransfer = (id: string, transferNo: string) => {
    try {
      ...
      showToastMsg(
        `Rejected and Cancelled Stock Transfer Request ${transferNo}.`,
        "info",
      );
    } catch (e) {
      console.error(e);
  ...
  if (currentUser.role === UserRole.ADMIN) {
    ...
    // Auto-reminder for sales reports based on operating schedule
    const todayNum = new Date().getDay(); // 0 = Sunday, 1 = Monday...
    const weekdays = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
    const todayStr = weekdays[todayNum];

    branches
      .filter((b) => !b.isDeleted)
      .forEach((br, index) => {
        const days = br.operatingDays || [
          "Mon",
          "Tue",
          "Wed",
          "Thu",
        ...
        if (days.includes(todayStr)) {
          const closeT = br.closingTime || "18:00";
          const openT = br.openingTime || "08:00";
          alarmsList.push({
            id: `auto-rem-report-${br.id}-${index}`,
            type: "warning",
            message: `Auto Reminder: ${br.name} is scheduled to operate today (${openT} - ${closeT}). Ensure its end-of-day Daily Sales Report has been compiled and transmitted.`,
          });
        }
      });
    ...
    const pendingCount = stockTransfers.filter(
      (t) => t.status === "Pending",
    ).length;
    if (pendingCount > 0) {
      alarmsList.push({
    ...
    const overduePOsCount = purchaseOrders.filter(
    ...
    return (
      <div className="space-y-6 animate-fade-in text-m3-on-surface">
        ...
        {toast && (
          <div
            className={`fixed top-4 right-4 z-50 p-4 rounded-2xl shadow-2xl flex items-center gap-3 border animate-slide-left ${
              toast.type === "success"
          ...
          >
            ...
            <button
              ...
              className="p-1 hover:bg-m3-on-surface/10 rounded-lg text-current"
            >
        ...
        <div className="android-glass border border-m3-outline-variant/30 rounded-[28px] p-6 shadow-lg bg-gradient-to-br from-m3-primary/5 to-m3-secondary/5 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
              <h2 className="text-2xl font-black mt-2 tracking-tight text-m3-primary">
                Enterprise Business Dashboard
              </h2>
              <p className="text-xs text-m3-on-surface-variant leading-relaxed mt-1 max-w-xl">
                Oversight terminal for{" "}
                <span className="font-bold text-m3-primary">Erica Manaban</span>
                . Admin clearing processes are live. Inter-branch stock
                redistribution triggers, purchase approvals, and company-wide
                sales analytics are consolidated.
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 shrink-0 items-stretch sm:items-end md:items-center w-full md:w-auto">
              <div className="flex flex-col gap-1 w-full sm:w-auto">
                <span className="text-[9px] text-m3-primary font-mono uppercase font-black pl-1">
                  Active View-Port Branch:
                </span>
                <select
                  ...
                  onChange={(e) => {
                    ...
                    showToastMsg(
                      `Switched Command View-Port to: ${e.target.value === "all" ? "All Corporate Branches" : getBranchName(e.target.value)}`,
                      "info",
                    );
                  }}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-black text-m3-on-surface uppercase tracking-wide flex items-center gap-2">
                  ...
                  Live Daily Sales Oversight Terminal
                </h3>
                <p className="text-[11px] text-m3-on-surface-variant font-mono mt-0.5">
                  Real-time transaction tracking, cashier clearing queues, and
                  multi-browser printed ledgers
                </p>
              </div>
            </div>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (activeProducts.length === 0) {
                      showToastMsg(
                        "Catalog is empty. Add products in the Inventory page first.",
                        "error",
                      );
                      return;
                    }
                    ...
                    for (let i = 0; i < count; i++) {
                      const idx = Math.floor(
                        Math.random() * activeProducts.length,
                      );
                    ...
                    try {
                      const subtotal = selectedItems.reduce(
                        (acc, it) =>
                          acc + it.product.sellingPrice * it.quantity,
                        0,
                      );
                      ...
                      showToastMsg(
                        `Simulated Sale Completed! Customer: ${randomCustomer} • Total: ₱${grandTotal.toLocaleString()}`,
                        "success",
                      );
                    } catch (err) {
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const todaySalesIds = new Set(
                    todaySalesItems.map((s) => s.id),
                  );
                  const todayBoxesSold = saleItems
                    .filter(
                      (si) => todaySalesIds.has(si.saleId) && !si.isDeleted,
                    )
                    .reduce((sum, item) => sum + item.quantity, 0);

                  const cashToday = todaySalesItems
                    .filter((s) => s.paymentMethod === "Cash")
                    .reduce((acc, s) => acc + s.grandTotal, 0);
                  const cardToday = todaySalesItems
                    .filter((s) => s.paymentMethod === "Credit Card")
                    .reduce((acc, s) => acc + s.grandTotal, 0);
                  const bankToday = todaySalesItems
                    .filter((s) => s.paymentMethod === "Bank Transfer")
                    .reduce((acc, s) => acc + s.grandTotal, 0);
                  const gcashToday = todaySalesItems
                    .filter(
                      (s) =>
                        s.paymentMethod === "GCash" ||
                        s.paymentMethod === "Maya",
                    )
                    .reduce((acc, s) => acc + s.grandTotal, 0);
 ...
 `;
                  ...
                  try {
                    ...
                    console.warn(
                      "Popup blocked. Switching to background print iframe fallback.",
                    );
                  }
                  ...
                  if (!popupSuccessful) {
                    try {
                      ...
                      const idoc = pIframe.contentWindow
                        ? pIframe.contentWindow.document
                        : pIframe.contentDocument;
                      if (idoc) {
                        idoc.open();
                  ...
                  showToastMsg(
                    "Daily Sales Ledger dispatched to print queue!",
                    "success",
                  );
                }}
            ...
            <div className="pt-5 space-y-6 animate-fade-in select-none">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
                <div className="bg-m3-surface-lowest p-4 rounded-2xl border border-m3-outline-variant/10 shadow-sm">
                  <span className="block text-[8px] font-extrabold text-m3-on-surface-variant/80 uppercase tracking-widest">
                    Live Today's Revenue
                  </span>
                  <div className="text-xl font-black mt-1 text-emerald-600 dark:text-emerald-400 tracking-tight">
                    ₱
                    {computedTodaySales.toLocaleString(undefined, {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-3.5">
                  ...
                  <div className="bg-m3-surface-lowest p-4.5 rounded-2xl border border-m3-outline-variant/10 space-y-4">
                    {[
                      {
                        label: "Cleared Cash drawer",
                        val: todaySalesItems
                          .filter((s) => s.paymentMethod === "Cash")
                          .reduce((acc, s) => acc + s.grandTotal, 0),
                        color: "bg-emerald-500",
                        count: todaySalesItems.filter(
                          (s) => s.paymentMethod === "Cash",
                        ).length,
                      },
                      {
                        label: "Card swipes volume",
                        val: todaySalesItems
                          .filter((s) => s.paymentMethod === "Credit Card")
                          .reduce((acc, s) => acc + s.grandTotal, 0),
                        color: "bg-m3-primary",
                        count: todaySalesItems.filter(
                          (s) => s.paymentMethod === "Credit Card",
                        ).length,
                      },
                      {
                        label: "GCash / Electronic Wallet",
                        val: todaySalesItems
                          .filter(
                            (s) =>
                              s.paymentMethod === "GCash" ||
                      ...
                      },
                      {
                        ...
                        color: "bg-pink-500",
                        count: todaySalesItems.filter(
                          (s) => s.paymentMethod === "Bank Transfer",
                        ).length,
                      },
                    ].map((mode, i) => {
                      ...
                      const percent = Math.min(
                        100,
                        Math.round((mode.val / totalVol) * 100),
                      );
                      return (
                        <div key={i} className="space-y-1.5">
                          <div className="flex justify-between items-end text-[10px]">
                            <span className="font-bold text-m3-on-surface-variant">
                              {mode.label}{" "}
                              <span className="text-[9px] text-m3-on-surface-variant/70 font-mono">
                                ({mode.count} POs)
                              </span>
                            </span>
                            <div className="text-right font-mono font-bold">
                              <span className="text-m3-on-surface mr-1.5">
                                ₱
                                {mode.val.toLocaleString(undefined, {
                                  maximumFractionDigits: 0,
                                })}
                              </span>
                              <span className="text-m3-on-surface-variant/80">
                                {percent}%
                              </span>
                            </div>
                          </div>
                ...
                <div className="space-y-3.5">
                  ...
                  <div className="bg-m3-surface-lowest p-4 rounded-2xl border border-m3-outline-variant/10 h-[190px] overflow-y-auto space-y-2.5 font-sans">
                    {(() => {
                      const cashierMap = todaySalesItems.reduce(
                        (acc, s) => {
                          acc[s.cashierName] =
                            (acc[s.cashierName] || 0) + s.grandTotal;
                          return acc;
                        },
                        {} as Record<string, number>,
                      );

                      const list = Object.entries(cashierMap).sort(
                      ...
                      if (list.length === 0) {
                        return (
                          <div className="text-center py-10 text-[10px] text-zinc-500 font-mono">
                            No cashier checkouts completed today. Simulated
                            transactions will populate this board instantly.
                          </div>
                        );
                      }
                      ...
                      return list.map(([name, sum], idx) => (
                        <div
                          key={idx}
                          className="flex items-center justify-between py-2 border-b border-m3-outline-variant/10 last:border-0"
                        >
              ...
              <div className="space-y-3 pt-2">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-m3-surface-lowest p-2 rounded-2xl border border-m3-outline-variant/10">
                  ...
                  <div className="flex items-center gap-1 px-2 shrink-0">
                    <span className="text-[9.5px] text-m3-on-surface-variant font-bold uppercase tracking-wider mr-1.5">
                      Payment Method:
                    </span>
                    {["all", "Cash", "Card", "GCash", "Bank Transfer"].map(
                      (m) => (
                        <button
                          key={m}
                          type="button"
                    ...
                    <table className="w-full text-left border-collapse m-0">
                      <thead>
                        <tr className="bg-m3-surface-low/30 text-[9px] uppercase tracking-wider text-m3-primary border-b border-m3-outline-variant/10">
                          ...
                          <th className="p-3 font-black text-right border-0">
                            Time
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {(() => {
                          const searchedTodaySales = todaySalesItems.filter(
                            (s) => {
                              ...
                              const matchesPayment =
                                activeDailyPaymentFilter === "all" ||
                                (s.paymentMethod || "").toLowerCase() ===
                                  activeDailyPaymentFilter.toLowerCase();
                              return matchesSearch && matchesPayment;
                            },
                          );

                          const visibleSales = showAllDailyTransactions
                            ? searchedTodaySales
                            : searchedTodaySales.slice(0, 5);
                          ...
                          if (visibleSales.length === 0) {
                            return (
                              <tr>
                                <td
                                  colSpan={6}
                                  className="p-8 text-center text-xs text-zinc-500 font-mono"
                                >
                                  No transaction filters match. Try clearing
                                  filters or simulate a customer checkout.
                                </td>
                              </tr>
                            );
                          }
                          ...
                          return visibleSales.map((s, i) => (
                            <tr
                              key={i}
                              className="border-b border-m3-outline-variant/10 hover:bg-m3-surface-low/30 last:border-none transition-colors"
                            >
                              <td className="p-3 text-xs font-mono font-bold text-m3-primary">
                                {s.saleNumber}
                              </td>
                              <td className="p-3 text-xs font-bold text-m3-on-surface">
                                {s.customerName}
                              </td>
                              <td className="p-3 text-xs font-semibold text-m3-on-surface-variant">
                                {s.cashierName}
                              </td>
                              <td className="p-3 text-[10px]">
                                <span
                                  className={`px-2 py-0.5 rounded-full font-bold uppercase tracking-wide text-[8.5px] ${
                                    s.paymentMethod === "Cash"
                                      ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20"
                                      : s.paymentMethod === "Credit Card"
                                        ? "bg-m3-primary/10 text-m3-primary border border-m3-primary/20"
                                        : s.paymentMethod === "GCash"
                                          ? "bg-m3-tertiary/10 text-m3-tertiary border border-m3-tertiary/30"
                              ...
                              <td className="p-3 text-xs font-black font-mono text-emerald-600 dark:text-emerald-400">
                                ₱
                                {s.grandTotal.toLocaleString(undefined, {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2,
                                })}
                              </td>
                    ...
                    if (searchedTodaySales.length > 5) {
                      return (
                        <div className="bg-m3-surface-low/10 p-2.5 text-center border-t border-m3-outline-variant/10">
                          ...
                          >
                            {showAllDailyTransactions
                              ? "Collapse transaction list"
                              : `View all remaining ${searchedTodaySales.length - 5} today tickets`}
                          </button>
                        </div>
                      );
                    }
          ...
          <div className="p-5.5 rounded-[24px] border border-m3-outline-variant/35 flex items-center justify-between shadow-md hover:shadow-lg transition-all hover:bg-m3-surface-low/90 duration-300 bg-m3-surface-low text-m3-on-surface relative overflow-hidden group">
            ...
            <div className="z-20">
              <span className="text-[10.5px] font-extrabold uppercase tracking-widest text-m3-on-surface-variant/80">
                {selectedBranchId === "all"
                  ? "Today's Sales (Global)"
                  : `Today's Sales (${getBranchName(selectedBranchId)})`}
              </span>
          ...
          <div className="p-5.5 rounded-[24px] border border-m3-outline-variant/35 flex items-center justify-between shadow-md hover:shadow-lg transition-all hover:bg-m3-surface-low/90 duration-300 bg-m3-surface-low text-m3-on-surface relative overflow-hidden group">
            ...
            <div className="z-20">
              <span className="text-[10.5px] font-extrabold uppercase tracking-widest text-m3-on-surface-variant/80">
                {selectedBranchId === "all"
                  ? "Monthly Revenue"
                  : `Month Revenue (${getBranchName(selectedBranchId)})`}
              </span>
                  ...
                  isEditingCorporateTarget ? (
                    ...
                    >
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setTempCorporateTargetInput(e.target.value)
                        }
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            ...
                            localStorage.setItem(
                              "tilepoint_corporate_target_v1",
                              String(val),
                            );
                            setCorporateTarget(val);
                            setIsEditingCorporateTarget(false);
                            showToastMsg(
                              "Corporate target updated successfully.",
                            );
                    ...
                    <div className="flex items-center gap-1">
                      <span>
                        Corporate Target:{" "}
                        <strong className="font-extrabold text-m3-on-surface font-sans">
                          ₱{corporateTarget.toLocaleString()}
                        </strong>
                      </span>
                      {currentUser.role === UserRole.ADMIN && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setTempCorporateTargetInput(
                              String(corporateTarget),
                            );
            ...
            <div className="z-20">
              <span className="text-[10.5px] font-extrabold uppercase tracking-widest text-m3-on-surface-variant/80">
                {selectedBranchId === "all"
                  ...
                  : `Stock Assets (${getBranchName(selectedBranchId)})`}
              </span>
              <div className="text-2xl font-black mt-2 tracking-tight text-amber-500">
                ₱
                {totalInventoryCostValue.toLocaleString(undefined, {
                  maximumFractionDigits: 0,
                })}
              </div>
              <div className="text-[10px] text-zinc-400 mt-1.5">
                Asset cost price basis valuation
              </div>
            </div>
          ...
          <div className="m3-card shadow-sm lg:col-span-2 flex flex-col justify-between">
              ...
              <div className="flex items-center justify-between mb-2">
                <div>
                  <h3 className="text-base font-extrabold tracking-tight text-m3-primary flex items-center gap-2">
                    <Building className="h-5 w-5 text-m3-primary" /> Branch
                    Performance Analytics
                  </h3>
                  <p className="text-xs text-m3-on-surface-variant">
                    Live sales aggregate versus baseline expectations
                  </p>
                </div>
                  ...
                  <tbody className="divide-y divide-m3-outline-variant/10">
                    {sortedBranchPerformance.map((b, idx) => {
                      ...
                      return (
                        <tr
                          key={idx}
                          className="hover:bg-m3-primary/5 transition-colors"
                        >
                          <td className="py-3">
                            {isEditing ? (
                              <div className="flex flex-col gap-2 p-2 bg-zinc-900 border border-m3-outline-variant/30 rounded-2xl animate-fade-in text-[11px] max-w-xs">
                                <span className="font-bold text-m3-primary uppercase font-mono">
                                  Adjust Parameters ({b.id})
                                </span>

                                <div className="flex flex-col gap-1">
                                  <label className="text-zinc-400 font-bold">
                                    Sales Target Quota (PHP):
                                  </label>
                                  <input
                                    type="number"
                                    value={editingBranchQuota}
                                    onChange={(e) =>
                                      setEditingBranchQuota(
                                ...
                                <div className="flex flex-col gap-1">
                                  <label className="text-zinc-400 font-bold">
                                    Floor Staff Assigned:
                                  </label>
                                  <input
                                    type="number"
                                    value={editingBranchStaff}
                                    onChange={(e) =>
                                      setEditingBranchStaff(
                                        Number(e.target.value),
                                      )
                                    }
                                    className="bg-zinc-800 border border-m3-outline-variant/20 rounded-xl p-1.5 text-xs text-white"
                                ...
                                <div className="flex gap-1.5 mt-1 justify-end">
                                  ...
                                  <button
                                    onClick={() => {
                                      ...
                                      showToastMsg(
                                        `Successfully updated branch operational boundaries for ${b.name}!`,
                                        "success",
                                      );
                                    }}
                              ...
                              <>
                                ...
                                <div className="text-[10.5px] text-zinc-400 mt-0.5 font-mono">
                                  Manager: {b.manager} • {b.staffCount} floor
                                  staff • Target: ₱
                                  {branchQuota.toLocaleString(undefined, {
                                    maximumFractionDigits: 0,
                                  })}
                                </div>
                              </>
                          ...
                          <td className="py-3 text-right font-mono font-bold text-m3-on-surface-variant align-middle">
                            ₱
                            {b.totalSales.toLocaleString(undefined, {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                          </td>
                          <td className="py-3 text-center align-middle">
                            <span
                              className={`inline-flex items-center gap-0.5 px-2.5 py-0.5 rounded-full font-bold font-mono text-[10px] ${
                                b.trend === "up"
            ...
            <div className="border-t border-m3-outline-variant/10 pt-4 mt-4 flex justify-between items-center bg-m3-surface-low/30 -mx-6 -mb-6 p-6 rounded-b-3xl">
              ...
              <button
                ...
                className="m3-btn text-xs font-bold py-1.5 flex items-center gap-1 select-none pr-3"
              >
          ...
          <div className="m3-card shadow-sm flex flex-col justify-between border-l-4 border-l-amber-500">
            <div>
              <h3 className="text-sm font-extrabold text-m3-primary tracking-tight uppercase tracking-wider flex items-center gap-1.5 mb-2.5">
                ...
                Enterprise Live Alerts
              </h3>
              <p className="text-[11.5px] text-m3-on-surface-variant mb-4">
                Urgent operational alerts requiring attention
              </p>
                ...
                {alarmsList.length === 0 && (
                  <div className="text-center py-8 text-xs text-m3-on-surface-variant flex flex-col items-center gap-1 bg-m3-surface-lowest/10 rounded-xl border border-dashed border-m3-outline-variant/20">
                    ...
                    <span className="font-extrabold text-emerald-500">
                      Systems 100% Normalized
                    </span>
                    <span className="text-[10px] text-zinc-400">
                      Zero active exceptions found
                    </span>
                  </div>
                )}
              </div>
            ...
            <div className="border-t border-m3-outline-variant/10 pt-4 mt-5 font-mono text-[10px] text-zinc-400 flex items-center justify-between">
              ...
              <button
                ...
                className="text-m3-primary hover:underline font-black cursor-pointer"
              >
                Employee Directory
              </button>
            </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-m3-outline-variant/15 pb-4 mb-4">
                <div>
                  ...
                  <p className="text-xs text-m3-on-surface-variant mt-0.5">
                    Top performing stock across transaction registers
                  </p>
                </div>
                  ...
                  <tbody className="divide-y divide-m3-outline-variant/10">
                    {bestsellingProducts.length === 0 ? (
                      <tr>
                        <td
                          colSpan={5}
                          className="py-8 text-center text-zinc-400 italic font-medium"
                        >
                          No items have been checkout simulated yet under this
                          branch/view-port.
                        </td>
                      </tr>
                    ) : (
                      bestsellingProducts
                        ...
                        .map((p, idx) => {
                          const formattedRev = p.revenue.toLocaleString(
                            undefined,
                            {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                          ...
                          return (
                            ...
                            >
                              <td className="py-3">
                                <div className="font-extrabold text-m3-on-surface">
                                  {p.productName}
                                </div>
                              ...
                              <td className="py-3 text-right pr-2 font-mono">
                                <span
                                  className={`font-bold ${p.stockQuantity <= 10 ? "text-rose-500" : "text-zinc-300"}`}
                                >
          ...
          <div className="m3-card shadow-sm border-l-4 border-l-emerald-500 flex flex-col justify-between text-m3-on-surface bg-m3-surface duration-300">
            <div>
              ...
              <p className="text-[11.5px] text-m3-on-surface-variant leading-relaxed mb-4">
                Smart broker matching branch stock shortages with branch
                overstocks.
              </p>
              ...
              <div className="space-y-3">
                ...
                {lowStockProducts
                  ...
                  .map((shortageProd, index) => {
                    ...
                    if (matchingExcessStock) {
                      const donorBranch = branches.find(
                        (b) => b.id === matchingExcessStock.branchId,
                      );
                      const destinationBranchId = activeBranchId || "B1"; // fall back to main branch context if none chosen
                      const localStock = getProductStockForCurrentContext(
                        shortageProd.id,
                      );
                      return (
                        <div
                          key={index}
                          className="p-3 bg-emerald-500/5 border border-emerald-500/15 rounded-xl space-y-2"
                        >
                          <div className="flex justify-between items-start">
                            <span className="text-[10px] text-emerald-400 font-black uppercase font-mono tracking-wider">
                              Branch Rebalance Match
                            </span>
                            <span className="text-[9.5px] font-mono font-bold bg-rose-500/20 text-rose-400 px-2 rounded-full">
                              Shortage ({localStock} left)
                            </span>
                          </div>
                          <p className="text-[11.3px] font-extrabold text-m3-on-surface truncate">
                            {shortageProd.productName}
                          </p>
                          <p className="text-[10px] text-zinc-400 leading-normal">
                            ...
                            <strong className="text-emerald-400">
                              {matchingExcessStock.quantity} boxes
                            </strong>
                            ).
                          </p>
                          <button
                            ...
                            onClick={() => {
                              try {
                                const items = [
                                  { productId: shortageProd.id, quantity: 15 },
                                ];
                                ...
                                showToastMsg(
                                  `Dispatched balancing transfer request for ${shortageProd.productName} from ${donorBranch?.name}!`,
                                  "success",
                                );
                              } catch (err) {
                                showToastMsg(
                                  "Rebalancing transfer dispatch rejected.",
                                  "error",
                                );
                              }
                            }}
                          ...
                          >
                            <ArrowRightLeft className="h-3 w-3" /> One-Click
                            Rebalance (15 boxes)
                          </button>
                        </div>
                      );
                ...
                {!lowStockProducts.length && !outOfStockProducts.length && (
                  <div className="p-3 bg-m3-surface-low/50 border border-m3-outline-variant/15 rounded-xl text-center py-7 text-xs text-m3-on-surface-variant border-dashed">
                    ...
                    <span className="font-bold text-zinc-350 block text-[11px]">
                      Balanced Stock Environment
                    </span>
                    <p className="text-[10px] text-zinc-400 mt-1 max-w-[190px] mx-auto leading-relaxed">
                      No urgent inter-branch stock balancing suggestions
                      matching your currently filtered viewport.
                    </p>
                  </div>
                )}
              </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-stretch">
          ...
          <div className="xl:col-span-8 m3-card shadow-sm flex flex-col justify-between">
              ...
              <div className="flex items-center justify-between mb-2">
                <div>
                  <h3 className="text-base font-extrabold text-m3-primary tracking-tight flex items-center gap-2">
                    ...
                    Active Logistics Transfer Hub
                  </h3>
                  <p className="text-xs text-m3-on-surface-variant">
                    Approve or Reject interlinked branch stock transport
                    requests
                  </p>
                </div>
              ...
              <div className="space-y-4 mt-4">
                ...
                {stockTransfers
                  ...
                  .map((t, idx) => (
                    <div
                      key={t.id || idx}
                      className="p-4 rounded-2xl border border-m3-outline-variant/30 bg-m3-surface-low hover:border-m3-primary transition-all flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-fade-in shadow-sm relative overflow-hidden"
                    >
                      <div className="absolute top-0 left-0 bottom-0 w-1 bg-amber-500" />
                      <div className="space-y-1.5 pl-2 max-w-lg">
                        <div className="flex items-center gap-2">
                          <span className="font-mono font-black text-xs text-amber-500 tracking-wide">
                            {t.transferNo}
                          </span>
                        ...
                        {t.items.map((it, itemIdx) => (
                          ...
                          >
                            ...
                            —{" "}
                            <span className="text-m3-primary font-black py-0.5 px-2 bg-m3-primary/10 rounded">
                              {it.quantity} boxes
                            </span>
                          </div>
                        ))}
                      ...
                      <div className="flex gap-2 w-full md:w-auto shrink-0 border-t md:border-0 pt-3 md:pt-0">
                        <button
                          onClick={() =>
                            handleDeclineTransfer(t.id, t.transferNo)
                          }
                        ...
                        <button
                          onClick={() =>
                            handleApproveTransfer(t.id, t.transferNo)
                          }
                ...
                {stockTransfers.filter((t) => t.status === "Pending").length ===
                  ...
                  <div className="text-center py-6 text-xs text-m3-on-surface-variant border border-dashed border-m3-outline-variant/20 rounded-2xl flex flex-col items-center gap-1 bg-emerald-500/5">
                    ...
                    <span className="font-bold text-emerald-500">
                      Zero Pending Approval Transfers
                    </span>
                    <span className="text-[10px] text-zinc-400">
                      All branch logistics requests are signed and cleared!
                    </span>
                  </div>
                )}
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                  {stockTransfers
                    ...
                    .map((t, idx) => (
                      <div
                        key={idx}
                        className="flex justify-between items-center text-xs p-3.5 rounded-xl border border-m3-outline-variant/10 hover:bg-m3-surface-low transition-colors"
                      >
                        <div>
                          <div className="flex items-center gap-2">
                          ...
                          <div className="text-zinc-400 text-xs font-mono mt-1">
                            {getBranchName(t.fromBranchId)}{" "}
                            {getBranchName(t.toBranchId)} • Approved by:{" "}
                            {t.approvedBy || "Admin"}
                          </div>
                        </div>
            ...
            <button
              onClick={() => onNavigate("transmittal")}
              className="m3-btn-tonal w-full mt-6 justify-center"
            >
          ...
          <div className="xl:col-span-4 m3-card shadow-sm flex flex-col justify-between">
            <div>
              <h3 className="text-sm font-extrabold text-m3-primary uppercase tracking-wider flex items-center gap-2 mb-1">
                <Layers className="h-4.5 w-4.5 text-m3-primary" /> Active
                Inventory Health
              </h3>
              <p className="text-xs text-m3-on-surface-variant mb-4">
                Click cards to drill down into product levels
              </p>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                ...
                <button
                  onClick={() =>
                    setActiveDrilldown(
                      activeDrilldown === "products" ? "none" : "products",
                    )
                  }
                ...
                >
                  <span className="text-[10px] text-zinc-400 block font-mono font-bold uppercase">
                    Total Products
                  </span>
                  <div className="text-lg font-black text-m3-on-surface mt-1">
                    {activeProducts.length}
                  </div>
                  <span className="text-[9px] text-m3-primary hover:underline block mt-1">
                    Drill Down{" "}
                  </span>
                </button>
                ...
                >
                  ...
                  <span className="text-[9px] text-amber-500 hover:underline block mt-1">
                    Drill Down{" "}
                  </span>
                </button>
                ...
                <button
                  onClick={() =>
                    setActiveDrilldown(
                      activeDrilldown === "critical" ? "none" : "critical",
                    )
                  }
                  className={`p-3 rounded-2xl border text-left transition-all relative overflow-hidden ${
                ...
                >
                  ...
                  <span className="text-[9px] text-rose-500 hover:underline block mt-1">
                    Drill Down{" "}
                  </span>
                </button>
                ...
                >
                  ...
                  <span className="text-[9px] text-red-500 hover:underline block mt-1">
                    Drill Down{" "}
                  </span>
                </button>
              </div>
              ...
              <div className="mt-4 border-t border-m3-outline-variant/15 pt-4">
                {activeDrilldown === "none" && (
                  <div className="text-center py-10 text-xs text-m3-on-surface-variant bg-m3-surface-low/30 rounded-2xl border border-dashed border-m3-outline-variant/15">
                    ...
                    Select any metric card above to view individual item
                    quantities, SKUs and replenish.
                  </div>
                )}
                ...
                {activeDrilldown !== "none" && (
                  <div className="space-y-3.5 max-h-76 overflow-y-auto pr-1 animate-fade-in">
                    ...
                    {(activeDrilldown === "products"
                      ...
                      const itemBranchStocks = branchStock.filter(
                        (bs) => bs.productId === p.id,
                      );

                      // Identify if any branch has excess stock (>25 boxes)
                      const excessBranches = itemBranchStocks.filter(
                        (bs) => bs.quantity > 25,
                      );
                      ...
                      return (
                        <div
                          key={idx}
                          className="p-3 bg-m3-surface-lowest rounded-xl border border-m3-outline-variant/15 text-xs flex flex-col justify-between gap-2.5"
                        >
                          ...
                          <div className="bg-m3-surface-low/50 p-2 rounded-lg space-y-1 my-1">
                            <span className="text-[9.5px] font-extrabold text-zinc-400 uppercase tracking-wide block">
                            ...
                            <div className="grid grid-cols-2 gap-1.5 text-[10.5px] font-mono">
                              {branches
                                ...
                                .map((br) => {
                                  const matchedStock =
                                    itemBranchStocks.find(
                                      (bs) => bs.branchId === br.id,
                                    )?.quantity || 0;
                                  const isLow =
                                    matchedStock < p.minimumStock / 2;
                                  return (
                                    <div
                                      key={br.id}
                                      className="flex justify-between items-center pr-2 bg-m3-surface-lowest p-1 rounded"
                                    >
                                      <span
                          ...
                          <div className="flex items-center justify-between border-t border-zinc-500/10 pt-2 text-[10.5px]">
                            <span>
                              ...
                              <span className="font-black text-m3-primary font-mono">
                                {p.stockQuantity} boxes
                              </span>
                            </span>
                            <span className="text-zinc-500 font-mono">
                              Min req: {p.minimumStock}
                            </span>
                          </div>
                            ...
                            {currentUser.role === "Admin" ? (
                              <div className="space-y-1">
                                <button
                                  onClick={() => {
                                    ...
                                    try {
                                      const cached =
                                        localStorage.getItem("tp_po_cart");
                                    ...
                                    showToastMsg(
                                      `Queued "${p.productName}" for Purchase Order! Navigating to Sourcing Desk...`,
                                      "success",
                                    );
                                    setTimeout(() => {
                                ...
                                {excessBranches.length > 0 && (
                                  <div className="pt-1">
                                    <p className="text-[10px] text-emerald-400 italic font-mono mb-1 text-center">
                                      Surplus Stock Available:
                                    </p>
                                    <div className="space-y-1">
                                      {excessBranches.map((eb) => {
                                        const donor = branches.find(
                                          (b) => b.id === eb.branchId,
                                        );
                                        const recipient =
                                          branches.find(
                                        ...
                                        return (
                                          <button
                                            ...
                                            onClick={() => {
                                              try {
                                                ...
                                                showToastMsg(
                                                  `Dispatched excess stock rebalance transfer of 15boxes from ${donor?.name}!`,
                                                  "success",
                                                );
                                              } catch (err) {
                                                showToastMsg(
                                                  "Failed to dispatch balancing transfer.",
                                                  "error",
                                                );
                                              }
                                            }}
                                          ...
                                          >
                                            Transfer 15 boxes from surplus{" "}
                                            {donor?.name}
                                          </button>
                                        );
                                      })}
                              ...
                              <div className="space-y-1">
                                <button
                                  onClick={() => {
                                    showToastMsg(
                                      `Manager request submitted: Restock requisition log for ${p.productName} initiated.`,
                                      "info",
                                    );
                                    let cart = [];
                                    try {
                                ...
                                {excessBranches.length > 0 && (
                                  <div className="pt-1">
                                    <p className="text-[10px] text-zinc-400 italic mb-1 text-center">
                                      Manage excess transfer option available:
                                    </p>
                                    {excessBranches.map((eb) => {
                                      const donor = branches.find(
                                        (b) => b.id === eb.branchId,
                                      ...
                                      return (
                                        <button
                                          ...
                                          onClick={() => {
                                            ...
                                            showToastMsg(
                                              `BALANCING: Sent request to transfer 15 units from ${donor?.name}.`,
                                              "success",
                                            );
                                          }}
                                        ...
                                        >
                                          Pull balancing stock transfer from{" "}
                                          {donor?.name}
                                        </button>
                                      );
                                    })}
                              ...
                              <div className="text-center text-[10.5px] text-zinc-500 py-1 font-sans italic border border-m3-outline-variant/10 rounded bg-m3-surface-low">
                                Restricted: Restocking requires Manager or Admin
                                clearance.
                              </div>
                            )}
                          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="relative h-56 w-full flex items-end pt-4 pb-2">
                <div className="absolute inset-y-0 left-0 flex flex-col justify-between text-[9px] text-m3-on-surface-variant font-mono pointer-events-none select-none">
                  <span>
                    {weeklyMetric === "revenue"
                      ? `₱${maxWeeklyAmount.toLocaleString()}`
                      : weeklyMetric === "orders"
                        ? `${maxWeeklyAmount} POs`
                        : `${maxWeeklyAmount} Boxes`}
                  </span>
                  <span>
                    {weeklyMetric === "revenue"
                      ? `₱${Math.round(maxWeeklyAmount * 0.5).toLocaleString()}`
                      : weeklyMetric === "orders"
                        ? `${Math.round(maxWeeklyAmount * 0.5)} POs`
                        : `${Math.round(maxWeeklyAmount * 0.5)} Boxes`}
                  </span>
                  <span>0</span>
                </div>
                ...
                <div className="pl-14 w-full h-full flex justify-between items-end gap-3 z-10">
                  {weeklyChartData.map((data, index) => {
                    const heightPercent = maxWeeklyAmount
                      ? (data.amount / maxWeeklyAmount) * 80
                      : 10;
                    ...
                    return (
                      <div
                        key={index}
                        onClick={() =>
                          setSelectedWeeklyDay(isSelected ? null : index)
                        }
                      ...
                      >
                        ...
                        {/* Hover Popup Tooltip */}
                        <div
                          className={`absolute bottom-full mb-3 bg-zinc-950/95 text-white text-[10px] py-1.5 px-2.5 rounded-xl border border-m3-outline-variant/35 font-bold font-mono shadow-2xl pointer-events-none transition-all duration-200 z-35 flex flex-col gap-0.5 items-center ${
                            hoveredBar === index
                        ...
                        >
                          ...
                          <span className="text-emerald-400 font-black">
                            {weeklyMetric === "revenue"
                              ...
                              : weeklyMetric === "orders"
                                ? `${data.amount} Orders`
                                : `${data.amount} Boxes Sold`}
                          </span>
                        </div>

                        <span
                          className={`text-[9.5px] font-mono mt-2 transition-colors ${isSelected ? "text-emerald-400 font-black" : "text-m3-on-surface-variant/80"}`}
                        >
                          {data.day}
                        </span>
                      </div>
          ...
          <div className="m3-card shadow-sm flex flex-col justify-between">
            <div>
                  ...
                  <label className="flex items-center gap-1.5 bg-zinc-805/90 px-2 py-1 border border-m3-outline-variant/20 rounded-xl cursor-pointer hover:bg-zinc-800 transition-colors">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        showToastMsg(
                          e.target.checked
                            ? "Exponential trend forecasting model overlay enabled!"
                            : "Extended forecasting deactivated.",
                          "info",
                        );
                      }}
              ...
              <div className="flex items-center justify-between p-2 rounded-xl bg-m3-tertiary-container/30 border border-m3-outline-variant/15 text-[10.5px] tracking-normal mb-3">
                ...
                <span className="font-extrabold text-m3-tertiary flex items-center gap-0.5 font-mono">
                  <TrendingUp className="h-3 w-3 animate-pulse" /> +14.5% Growth
                  Profile
                </span>
              </div>
              ...
              <div className="relative h-56 w-full pt-4">
                <div className="absolute inset-y-0 left-0 flex flex-col justify-between text-[9px] text-m3-on-surface-variant font-mono pointer-events-none select-none">
                  <span>
                    ₱
                    {maxMonthlyAmount.toLocaleString(undefined, {
                      notation: "compact",
                    })}
                  </span>
                  <span>
                    ₱
                    {Math.round(maxMonthlyAmount * 0.5).toLocaleString(
                      undefined,
                      { notation: "compact" },
                    )}
                  </span>
                ...
                <div className="ml-14 h-40 relative border-b border-m3-outline-variant/20">
                  ...
                  >
                    ...
                    <path d={svgPaths.areaD} fill="url(#adminWaveGrad)" />
                        ...
                        <circle
                          ...
                          onClick={() => {
                            setSelectedForecastMonth(
                              selectedForecastMonth === idx ? null : idx,
                            );
                            showToastMsg(
                              `Drilled down details for ${pt.month}: ₱${pt.revenue.toLocaleString()} model projections.`,
                              "info",
                            );
                          }}
                        />
                  ...
                  <div className="absolute w-full top-full pt-1.5 flex justify-between text-[9px] font-mono font-bold text-m3-on-surface-variant/80">
                    {monthlyChartData.map((d, i) => (
                      <span
                        ...
                        onClick={() =>
                          setSelectedForecastMonth(
                            selectedForecastMonth === i ? null : i,
                          )
                        }
                  ...
                  {hoveredPoint !== null && svgPaths.points[hoveredPoint] && (
                    ...
                    >
                      <span className="text-[8.5px] text-zinc-400">
                        {monthlyChartData[hoveredPoint].month}
                      </span>
                      <span className="text-m3-tertiary font-black">
                        ₱
                        {monthlyChartData[
                          hoveredPoint
                      ...
                      {monthlyChartData[hoveredPoint].isPredicted && (
                        <span className="text-[8px] text-emerald-400 font-bold tracking-wider uppercase mt-0.5">
                          Estimated Forecast
                        </span>
                      )}
                    </div>
            ...
            {selectedForecastMonth !== null &&
              monthlyChartData[selectedForecastMonth] && (
                <div className="p-3 bg-zinc-800/60 border border-m3-outline-variant/20 rounded-2xl animate-fade-in flex flex-col mt-4 select-none shrink-0">
                  <div className="flex justify-between items-center text-[10.5px]">
                    <span className="text-m3-primary font-bold">
                      ...
                      {monthlyChartData[selectedForecastMonth].month})
                    </span>
                    <button
                  ...
                  <div className="grid grid-cols-3 gap-3 mt-2 text-center font-mono">
                    <div className="p-2.5 rounded-xl bg-zinc-900 border border-m3-outline-variant/15">
                      ...
                      <span className="text-[11px] font-black text-white mt-1 block">
                        {monthlyChartData[selectedForecastMonth].isPredicted
                          ? "ESTIMATION"
                          : "HISTORIC"}
                      </span>
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8 m3-card shadow-sm flex flex-col justify-between">
              ...
              <div className="flex items-center justify-between mb-2">
                <div>
                  <h3 className="text-base font-extrabold tracking-tight text-m3-primary flex items-center gap-1.5">
                    <History className="h-5 w-5 text-m3-primary" /> Inventory
                    Aging & Redistribution Suggestion Feed
                  </h3>
                  <p className="text-xs text-m3-on-surface-variant font-mono">
                    Slow-moving tiles candidates targeted for reallocation
                  </p>
                </div>
                ...
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-m3-outline-variant/10">
                    {slowMovingCandidates.map((cand, idx) => (
                      ...
                      >
                        <td className="py-3">
                          <div className="font-extrabold text-m3-on-surface leading-tight">
                            {cand.productName}
                          </div>
                          <div className="text-[10px] text-zinc-400 font-mono mt-0.5">
                            Code Segment: {cand.productId}
                          </div>
                        </td>
                        ...
                        <td className="py-3 text-center">
                          <span
                            className={`px-2 py-0.5 rounded text-[9.5px] font-black font-mono uppercase ${
                              cand.riskLevel === "HIGH"
                        ...
                        <td className="py-3 text-right">
                          <div className="flex flex-col items-end gap-1">
                            <span className="text-[10px] font-bold text-emerald-400 font-mono italic">
            ...
            <p className="text-[10.5px] text-zinc-400 mt-4 pl-1 font-mono italic">
              *AI Model monitors slow stock sales profiles over 90 days to
              release dynamic pull out proposals.
            </p>
          </div>
          ...
          <div className="lg:col-span-4 m3-card shadow-sm flex flex-col justify-between">
            <div>
              <h3 className="text-sm font-extrabold text-m3-on-primary-container uppercase tracking-wider flex items-center gap-1.5 mb-1 bg-m3-primary-container p-3 rounded-t-2xl -mx-6 -mt-6">
                <ClipboardList className="h-4.5 w-4.5 text-m3-on-primary-container" />{" "}
                Supply Procurement Pipeline
              </h3>
              <p className="text-xs text-m3-on-surface-variant mt-4 mb-4">
                Manufacturer connections & incoming inventory purchase ledgers
              </p>

              <div className="grid grid-cols-2 gap-3 mt-2">
                <div className="p-3 rounded-xl bg-m3-surface-low border border-m3-outline-variant/30 text-left font-mono">
                  <span className="text-[9.5px] text-zinc-400 block font-bold">
                    PENDING POs
                  </span>
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-base font-extrabold text-m3-primary flex items-center gap-2">
                <ShieldCheck className="h-5 w-5 text-m3-primary" /> Global
                Enterprise Security Audit Stream
              </h3>
              <p className="text-xs text-m3-on-surface-variant">
                Live audit ledger tracking voided sales, manager code approvals
          ...
          <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
            {auditLogs.slice(0, 12).map((log, idx) => {
              ...
              const isSuccess =
                ...
                actionText.includes("RECEIVE") ||
                actionText.includes("SUCCESS");
              const isInfo =
              ...
              return (
                <div
                  key={idx}
                  className="flex justify-between items-start text-xs border-b border-m3-outline-variant/10 pb-2.5 last:border-0 last:pb-0 hover:bg-m3-surface-low rounded p-1 transition-colors"
                >
                  <div className="space-y-1 pr-4">
                    <span
                      className={`text-[9.5px] uppercase tracking-wider font-bold inline-block px-2.5 py-0.5 rounded font-mono border ${
                        isDanger
                          ? "bg-rose-500/10 text-rose-400 border-rose-500/20"
                          : isSuccess
                            ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20"
                    ...
                    <span className="text-m3-on-surface font-medium block leading-snug">
                      {log.description}
                    </span>
                    <span className="text-[10px] text-zinc-400 block font-mono pl-1">
                      <span className="hidden sm:inline">
                        Target Record: {log.tableAffected} (
                        {log.recordId || "Global"}) •{" "}
                      </span>
            ...
            {auditLogs.length === 0 && (
              <div className="text-center py-6 text-xs text-m3-on-surface-variant">
                No system operations tracked yet.
              </div>
            )}
          </div>
        </div>
    ...
    <div className="space-y-6 animate-fade-in text-m3-on-surface">
        ...
        <div className="lg:col-span-12">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-m3-on-surface-variant font-mono">
              Active Store Dashboard Context
            </span>
          </div>

          <div className="animate-fade-in mt-3">
            <h2 className="text-xl font-extrabold tracking-tight text-m3-primary">
            ...
            <p className="text-xs text-m3-on-surface-variant leading-relaxed mt-1">
              ...
              . Review local store parameters, handle shift operations, process
              customer checkout baskets, or perform inventory checkouts below.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 mt-4">
              {currentUser.role !== UserRole.CASHIER && (
                <button
                  onClick={() => onNavigate("inventory")}
                  className="p-2.5 text-left text-xs bg-m3-surface-low rounded-xl border border-m3-outline-variant/35 hover:bg-m3-primary/10 hover:text-m3-primary transition-all flex justify-between items-center cursor-pointer"
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {outOfStockProducts.length > 0 && (
            <div className="bg-rose-500/10 dark:bg-rose-950/30 border border-rose-500/20 dark:border-rose-500/10 border-l-4 border-l-rose-600 dark:border-l-rose-400 p-5 rounded-[24px] rounded-l-none flex items-start gap-4 shadow-md duration-250 hover:shadow-lg transition-all">
              ...
              <div>
                <h4 className="text-sm font-extrabold text-rose-800 dark:text-rose-200">
                  Out of Stock Alert
                </h4>
                <p className="text-xs text-rose-700 dark:text-rose-300 mt-1.5 leading-relaxed font-semibold">
                  There are{" "}
                  <span className="font-extrabold text-rose-900 dark:text-white bg-rose-500/10 dark:bg-rose-500/20 px-1.5 py-0.5 rounded-md">
                    {outOfStockProducts.length}
                  </span>{" "}
                  items completely depleted in local store records.
                </p>
                <button
                  onClick={() => onNavigate("inventory")}
            ...
            <div className="bg-amber-500/10 dark:bg-amber-950/30 border border-amber-500/20 dark:border-amber-500/10 border-l-4 border-l-amber-600 dark:border-l-amber-400 p-5 rounded-[24px] rounded-l-none flex items-start gap-4 shadow-md duration-250 hover:shadow-lg transition-all">
              ...
              <div>
                ...
                <p className="text-xs text-amber-700 dark:text-amber-300 mt-1.5 leading-relaxed font-semibold">
                  Local stock for{" "}
                  <span className="font-extrabold text-amber-900 dark:text-white bg-amber-500/10 dark:bg-amber-500/20 px-1.5 py-0.5 rounded-md">
                    {lowStockProducts.length}
                  </span>{" "}
                  items have fallen below safety limits. Restock required.
                </p>
                <button
                  onClick={() => onNavigate("procurement")}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 rounded-[24px] border border-m3-outline-variant/35 flex items-center justify-between shadow-sm transition-all hover:bg-m3-surface-low duration-250 bg-m3-surface-low text-m3-on-surface glowing-card">
          <div>
            <span className="text-[11px] font-bold uppercase tracking-widest text-m3-on-surface-variant/75">
              Total Products
            </span>
            <div className="text-3xl font-black mt-1.5 tracking-tight text-m3-primary">
              {stats.totalProducts}
            </div>
            <div className="text-[10px] text-m3-on-surface-variant/80 mt-1 font-mono">
        ...
        <div className="p-5 rounded-[24px] border border-m3-outline-variant/35 flex items-center justify-between shadow-sm transition-all hover:bg-m3-surface-low duration-250 bg-m3-surface-low text-m3-on-surface glowing-card">
          <div>
            ...
            <div className="text-[10px] text-m3-on-surface-variant/80 mt-1">
              Target ₱2.0M • Local hub
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="m3-card shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-semibold tracking-tight text-m3-primary">
                Weekly Sales Trend
              </h3>
              <p className="text-xs text-m3-on-surface-variant">
                Sales count value per day (PHP)
              </p>
            </div>
            <span className="bg-m3-primary-container text-m3-on-primary-container font-mono px-3 py-1 rounded-full text-[11px] font-bold border border-m3-outline-variant/30">
              ₱
              {stats.weeklySales.toLocaleString(undefined, {
                maximumFractionDigits: 0,
              })}{" "}
              Total
            </span>
              ...
              {weeklyChartData.map((data, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <div
                      className={`absolute bottom-full mb-2 bg-m3-on-surface text-m3-surface text-[10.5px] py-1.5 px-3 rounded-xl border border-m3-outline-variant/30 font-bold font-mono shadow-lg pointer-events-none transition-opacity duration-200 z-35 ${
                        hoveredBar === index ? "opacity-100" : "opacity-0"
                      }`}
                    >
                      ₱{data.amount.toLocaleString()}
                    </div>
                    <span className="text-[10px] font-semibold text-m3-on-surface-variant/80 mt-2">
                      {data.day}
                    </span>
                  </div>
                );
        ...
        <div className="m3-card shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-m3-on-surface-variant">
                Local branch aggregate progression (PHP)
              </p>
            </div>
          ...
          <div className="relative h-64 w-full pt-4">
            <div className="absolute inset-y-0 left-0 flex flex-col justify-between text-[10px] text-m3-on-surface-variant font-mono pointer-events-none">
              ...
              <span>
                ₱
                {(maxMonthlyAmount * 0.5).toLocaleString(undefined, {
                  notation: "compact",
                })}
              </span>
              <span>₱0</span>
 ...
 C 510 ${200 - getMonthlyRatio(monthlyChartData[4].revenue) * 160}, 530 ${200 - getMonthlyRatio(monthlyChartData[5].revenue) * 160}, 570 ${200 - getMonthlyRatio(monthlyChartData[5].revenue) * 160}`}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="m3-card shadow-sm flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-m3-on-surface-variant mb-4">
              Leading local stock classifications
            </p>
                ...
                return (
                  <div key={idx} className="space-y-1">
                    <div className="flex justify-between text-xs font-semibold">
                      <span>{cat.name}</span>
                      <span className="font-mono text-m3-on-surface-variant">
                        {cat.count} lines
                      </span>
                    </div>
                    <div className="w-full bg-m3-outline-variant/20 h-2.5 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, (Number(cat.count) / Number(stats.totalProducts)) * 100)}%`,
                        }}
                      />
                    </div>
                  </div>
        ...
        <div className="m3-card shadow-sm flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-m3-on-surface-variant mb-4">
              Latest checkout invoices saved
            </p>
            ...
            <div className="space-y-3">
              {filteredSales.slice(0, 4).map((sale, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between py-2.5 border-b border-m3-outline-variant/10 last:border-0 hover:bg-m3-surface-low rounded px-1.5 transition-colors"
                >
                  <div>
                  ...
                  <div className="text-right">
                    <div className="text-xs font-bold font-mono text-m3-tertiary">
              ...
              {filteredSales.length === 0 && (
                <div className="text-center py-6 text-xs text-m3-on-surface-variant">
                  No transactions details of today yet.
                </div>
              )}
            </div>
          </div>
        ...
        <div className="m3-card shadow-sm flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-m3-on-surface-variant mb-4">
              Local purchase orders pending delivery
            </p>
            ...
            <div className="space-y-3">
              {pendingOrders.slice(0, 4).map((po, idx) => (
                ...
                >
                  <div>
                    <div className="text-xs font-bold font-mono text-m3-on-surface">
                      {po.poNumber}
                    </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <div className="m3-card shadow-sm flex flex-col justify-between animate-fade-in">
          ...
          <p className="text-[10.5px] text-zinc-400 mt-4 pl-1 font-mono italic">
            *AI Model monitors slow stock sales profiles over 90 days to release
            dynamic pull out proposals.
          </p>
        </div>
