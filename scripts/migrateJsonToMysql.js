...
async function migrateData(dataInput, customConfig = {}) {
  ...
  const _purchaseOrders = data.tp_purchase_orders || data.purchaseOrders || [];
  const _poItems = data.tp_po_items || data.poItems || [];
  const _stockTransfers = data.tp_stock_transfers || data.stockTransfers || [];
  const _stockTransferItems = data.tp_stock_transfer_items || data.stockTransferItems || [];
  const _movements = data.tp_movements || data.movements || [];
  const _inventoryMovements = data.tp_inventory_movements || data.inventoryMovements || [];
  const _deliveries = data.tp_deliveries || data.deliveries || [];
  const _damageLogs = data.tp_damage_logs || data.damageLogs || [];
  const _ledgerEntries = data.tp_ledger_entries || data.ledgerEntries || [];
  const _auditLogs = data.tp_audit_logs || data.auditLogs || [];
  const _customCorporateBills = data.tp_custom_corporate_bills || data.customCorporateBills || [];
  const _transmittals = data.tp_transmittals || data.transmittals || [];
  const _members = data.tp_members || data.members || [];
  const _expenses = data.tp_expenses || data.expenses || [];
  const _productReturns = data.tp_product_returns || data.productReturns || [];
  const _branchSalesReports = data.tp_branch_sales_reports || data.branchSalesReports || [];
